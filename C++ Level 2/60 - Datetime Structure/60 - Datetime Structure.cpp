#pragma warning(disable : 4996)
#include <iostream>
#include <ctime>
using namespace std;


int main()
{
	// Get the current time
	time_t t = time(0);

	// Converts the time to a tm structure that contains all the date and time information
	tm* now = localtime(&t);

	// tm_year gives the number of years since 1900
	// So we add 1900 to get the actuall year
	cout << "Year : " << now->tm_year + 1900 << endl;
	// tm_mon gives the month from 0 to 11
	// So we add 1 to get the actual month from 1 to 12
	cout << "Month : " << now->tm_mon + 1 << endl;


	cout << "Day : " << now->tm_mday << endl;
	cout << "Hour : " << now->tm_hour << endl;
	cout << "Min : " << now->tm_min << endl;
	cout << "Second : " << now->tm_sec << endl;
	cout << "Week Day (Days since sunday) : " << now->tm_wday << endl;
	cout << "Year Day (Days since Jan 1st) : " << now->tm_yday << endl;
	cout << "daylight savings time : " << now->tm_isdst << endl;
}


/*
	int tm_sec; // seconds of minutes from 0 to 60
	int tm_min; // minutes of hour from 0 to 59
	int tm_hour; // hours of day from 0 to 23
	int tm_mday; // day of month from 1 to 31
	int tm_mon; // month of year from 0 to 11
	int tm_year; // year since 1900
	int tm_wday; // days since sunday
	int tm_yday; // days since January 1st
	int tm_isdst; // indicates whether daylight saving time is active
*/